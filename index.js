...
app.post('/auto-generate-clips', async (req,res)=>{
  ...
  for(const act of actions){
    const seg = segments.find(s=>{
      ...
      return act.timestamp_in_game >= s0 && act.timestamp_in_game < s0 + Number(s.duration||20);
    });
    if(!seg){ console.warn('no segment'); continue; }
...
app.get('/clips', async (_,res)=>{
  ...
  res.json(list.data.files.map(f=>({
    external_id:f.id,
    name:f.name,
